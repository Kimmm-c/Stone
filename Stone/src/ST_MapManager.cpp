...
int ST_MapManager::loadMap( const ST_MapContext& context, SDL_Texture* texture )
{
    ...
    for (int row = 0; row < mapHeight; row++)
    {
        for (int col = 0; col < mapWidth; col++)
        {
            ...
            // Create entity
            ST_Entity& tile = context.parentLayer.createEntity();

            // Set up Sprite component for each tile
            Sprite texture;
            texture.src.w = texture.dest.w = context.tileWidth;
            texture.src.h = texture.dest.h = context.tileHeight;

            int textureIndex = std::stoi( tile );

            tileMap[row][col] = std::stoi( tile ); // stoi: convert string to int
        }
    }
